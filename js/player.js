...
function initPlayer(videoUrl) {
        ...
        customType: {
            m3u8: function (video, url) {
                ...
                hls.on(Hls.Events.MANIFEST_PARSED, function (eventName, data) {
                    ...
                    // 尝试从 hls.levels 中读取最高分辨率（多码率流场景）
                    try {
                        const levels = hls.levels;
                        if (levels && levels.length > 0) {
                            // 取分辨率最高的 level
                            const best = levels.reduce((prev, cur) =>
                                ((cur.width * cur.height) > (prev.width * prev.height)) ? cur : prev
                            );
                            if (best.width && best.height) {
                                showResolutionBadge(best.width, best.height);
                            }
                        }
                    } catch (e) { }
                });
    ...
    art.on('video:loadedmetadata', function () {
        ...
        // 兜底：如果 MANIFEST_PARSED 未能获取分辨率，使用原生 video 元素的宽高
        const resEl = document.getElementById('videoResolution');
        if (resEl && resEl.classList.contains('hidden')) {
            const vw = art.video && art.video.videoWidth;
            const vh = art.video && art.video.videoHeight;
            if (vw && vh) {
                showResolutionBadge(vw, vh);
            }
        }
...
/**
 * 在播放器标题栏旁显示真实视频分辨率徽章
 * @param {number} w - 宽度像素值
 * @param {number} h - 高度像素值
 */
function showResolutionBadge(w, h) {
